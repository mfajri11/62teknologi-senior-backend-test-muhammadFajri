...
type BusinessSearchRepository struct {
	sqlClient postgres.SQLer
}

func NewBusinessSearchRepository(db postgres.SQLer) *BusinessSearchRepository {
	return &BusinessSearchRepository{sqlClient: db}
}

func (repo *BusinessSearchRepository) DeleteByID(ctx context.Context, id string) (int64, error) {
	query := `
	WITH 
		basic AS 
	...
	if err != nil {
		err = fmt.Errorf("repository.BusinessSearchRepository.DeleteByID: error exec delete query: %w", err)
		return 0, err
	}
...
func (repo *BusinessSearchRepository) prepareArgsUpsertByID(business model.BusinessSearchUpsertQuery) []interface{} {
...
func (repo *BusinessSearchRepository) UpdateByID(ctx context.Context, business model.BusinessSearchUpsertQuery) (int64, error) {
	query := `
	WITH basic AS (
	...
	args := repo.prepareArgsUpsertByID(business)
	res, err := repo.sqlClient.Exec(ctx, query, args...)

	if err != nil {
		err := fmt.Errorf("repository.BusinessSearchRepository.UpdateByID: error exec update query: %w", err)
		return 0, err
	}
...
func (repo *BusinessSearchRepository) Create(ctx context.Context, business model.BusinessSearchUpsertQuery) (id string, err error) {
	query := `
	WITH basic AS (
	...
	if err != nil {
		err := fmt.Errorf("repository.BusinessSearchRepository.Create: error exec insert query: %w", err)
		return "", err
	}
